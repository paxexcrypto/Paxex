...
#if defined(NDEBUG)
#error "Paxex cannot be compiled without assertions."
#endif
...
/** Fees smaller than this (in upaxex) are considered zero fee (for relaying and mining)
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == "version") {
        ...
        // PAXEX: We use certain sporks during IBD, so check to see if they are
