...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("PaxexMiner : generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, NULL, pblock))
        return error("PaxexMiner : ProcessNewBlock, block not accepted");
...
{
    LogPrintf("PaxexMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("paxex-miner");
    ...
    while (fGenerateBitcoins || fProofOfStake) {
        ...
        LogPrintf("Running PaxexMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
